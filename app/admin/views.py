...
class JobAdmin(ModelView, model=Job):
    column_list = [Job.title, Job.organization, Job.state, Job.notice_url, Job.status, Job.published_at, Job.updated_at]
    column_searchable_list = [Job.title, Job.organization, Job.state]
    column_filters = [Job.status, Job.organization, Job.state]
...
class RawItemAdmin(ModelView, model=RawItem):
    column_list = [RawItem.title_raw, RawItem.url_raw, RawItem.status, RawItem.found_at]
